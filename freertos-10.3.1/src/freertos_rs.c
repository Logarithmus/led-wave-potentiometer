...
uint8_t freertos_rs_sizeof(uint8_t type) {
	switch (type) {
		case 0: return sizeof(void*); break;
		case 1: return sizeof(char*); break;
		case 2: return sizeof(char); break;
		case 10: return sizeof(BaseType_t); break;
		case 11: return sizeof(UBaseType_t); break;
		case 12: return sizeof(TickType_t); break;		
		case 20: return sizeof(TaskHandle_t); break;
		case 21: return sizeof(QueueHandle_t); break;
		case 22: return sizeof(SemaphoreHandle_t); break;
		case 23: return sizeof(TaskFunction_t); break;
		case 24: return sizeof(TimerHandle_t); break;
		case 25: return sizeof(TimerCallbackFunction_t); break;
		case 30: return sizeof(TaskStatus_t); break;
		case 31: return sizeof(eTaskState); break;
		case 32: return sizeof(unsigned long); break;
		case 33: return sizeof(unsigned short); break;
		default: return 0;
	}
}
...
UBaseType_t freertos_rs_take_recursive_mutex(QueueHandle_t mutex, UBaseType_t max) {
	return (xSemaphoreTakeRecursive(mutex, max) == pdTRUE) ? 0 : 1;
}
...
UBaseType_t freertos_rs_take_mutex(QueueHandle_t mutex, UBaseType_t max) {
	return (xSemaphoreTake(mutex, max) == pdTRUE) ? 0 : 1;
}
...
UBaseType_t freertos_rs_take_semaphore_isr(QueueHandle_t semaphore, BaseType_t* xHigherPriorityTaskWoken) {
	return (xSemaphoreTakeFromISR(semaphore, xHigherPriorityTaskWoken) == pdTRUE) ? 0 : 1;
}

UBaseType_t freertos_rs_give_semaphore_isr(QueueHandle_t semaphore, BaseType_t* xHigherPriorityTaskWoken) {
	return (xSemaphoreGiveFromISR(semaphore, xHigherPriorityTaskWoken) == pdTRUE) ? 0 : 1;
}
...
UBaseType_t freertos_rs_get_stack_high_water_mark(__attribute__((unused)) TaskHandle_t task) {
...
BaseType_t freertos_rs_task_notify_wait(uint32_t ulBitsToClearOnEntry, uint32_t ulBitsToClearOnExit, uint32_t *pulNotificationValue, TickType_t xTicksToWait) {
	return (xTaskNotifyWait(ulBitsToClearOnEntry, ulBitsToClearOnExit, pulNotificationValue, xTicksToWait) == pdTRUE) ? 0 : 1;
}

eNotifyAction freertos_rs_task_notify_action(uint8_t action) {
	switch (action) {					
		case 1:  return eSetBits;
		case 2:  return eIncrement;
		case 3:  return eSetValueWithOverwrite;
		case 4:  return eSetValueWithoutOverwrite;
		default: return eNoAction;
	}
}
